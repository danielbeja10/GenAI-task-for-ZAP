...
1. **שגיאות כתיב** — עלולות להיות שגיאות בעת רשימת המוצרים; המודל מתקן אך ורק כאשר אין דו-משמעות.
2. **רמת פירוט משתנה** — מוצרים יכולים להיות שמות כלליים מאוד כמו `samsung galaxy` ועד שמות ספציפיים כמו `samsung galaxy s23 264gb blue`. לכן המודל אינו מוסיף מידע מדעתו, ומבדיל בין דגמים זהים בעלי מאפיינים חומרתיים שונים (לדוגמה: `samsung galaxy s23 264gb blue` לעומת `samsung galaxy s23 128gb blue`).
3. **פורמט הרשימה** — כל רשימה נראית אחרת; ההפרדה בין מוצרים יכולה להיות בנקודות, פסיקים, כוכביות, שורות אנכיות, או רשימה אופקית. הפרומפט מתמודד עם כל הפורמטים הללו.
...
הפלט צריך להיות חד וספציפי כדי שנוכל לנתח אותו בקוד. לכן הנחיתי את המודל להחזיר תשובה בפורמט JSON בלבד, ללא כל תוספת טקסט. כך ניתן לטעון את הפלט ישירות לקוד ולבצע עליו מניפולציות בצורה נוחה.

#### שלב 3 — דוגמאות (Few-shot Prompting)

השתמשתי ב-few-shot prompting: הצגתי למודל הן את מבנה ה-JSON הרצוי והן מספר דוגמאות של input/output בדיוק כפי שנדרש. כך מונעים אי-הבנות ומחדדים את הדגשים שניתנו.
...
**גישה 1 — כתיבה ידנית**
כתבתי בעצמי את כל הנקודות החשובות, שלחתי למודל, והוא החזיר אותן מסודרות ומנוסחות בצורה יעילה.
- יתרון: שליטה מלאה על הפרומפט ועל השיח מול המודל.
- חסרון: אין ודאות שהדגשים מספיקים, ועלול להיות דאטה שיפיל את הפרומפט.
...
- חסרונות: מצריך כמות גדולה של דאטה; עלול ליצור overfitting ולשגות על דאטה חדש.
...
שיקול חשוב: חשבתי תחילה לשלוף את המחיר ישירות מ-ZAP, אך לאחר בדיקה הבנתי שקריאות סיסטמתיות לאתר עלולות לגרום לחסימה. לכן הסתפקתי ביצירת לינק בלבד.
